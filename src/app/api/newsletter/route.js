export async function POST(req) {
  try {
    ...
    if (!API_KEY || !LIST_ID) {
      ...
      return new Response(
        JSON.stringify({ error: "Newsletter service is not configured properly" }),
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({ 
          error: data.error?.message || "Failed to subscribe to the newsletter"
        }),
    ...
    return new Response(
      JSON.stringify({ error: "Newsletter subscription failed" }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
  }
}